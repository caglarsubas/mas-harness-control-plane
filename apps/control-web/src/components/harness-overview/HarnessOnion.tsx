...
function move(event: KeyboardEvent<HTMLAnchorElement>, index: number): void {
  ...
  const links = [...event.currentTarget.closest('[data-onion-navigation="true"]')?.querySelectorAll<HTMLAnchorElement>("a[data-onion-node]") ?? []];
...
export function HarnessOnion({ summaries, visibleIds }: { readonly summaries: readonly HarnessSummary[]; readonly visibleIds: ReadonlySet<string> }) {
  return (
    <section className={styles.onionPanel} aria-labelledby="system-map-title">
      ...
      <nav className={styles.onion} aria-label="Sixteen harnesses arranged by plane" data-onion-navigation="true">
        ...
        {HARNESSES.map((definition, index) => {
          ...
          const filterMatch = visibleIds.has(definition.id);
          const label = summary ? `${definition.name}: ${summary.aggregateState}; ${summary.freshnessState}; ${summary.blockerCount} blockers; ${filterMatch ? "matches" : "outside"} current filters` : `${definition.name}: unavailable`;
          return (
            <a
              aria-label={label}
              className={styles.onionNode}
              data-filter-match={filterMatch}
              data-onion-node
